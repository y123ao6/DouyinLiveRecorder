    ...
    def _get_dynamic_status_info(self):
        ...
        if os.path.exists(self.main_config_file):
            try:
                ...
                # 保持键的大小写敏感（可选，视具体ini内容而定）
                config.optionxform = str 
                config.read(self.main_config_file, encoding='utf-8-sig')

                # 1. 读取循环检测时间 (假设键名为 check_interval 或类似)
                # 注意：这里需要根据你实际的 config.ini 中的键名进行调整
                if '循环时间(秒)' in config: # 假设配置在 [Settings] 块下
                    interval = config['循环时间(秒)'].get('check_interval', '120')
                    check_interval = f"{interval}秒"
                
                # 2. 读取输出格式 (假设键名为 output_format 或 video_format)
                # 这里做一个简单的映射示例
                fmt = config['录制完成后自动转为mp4格式'].get('output_format', 'mp4')
                if fmt.lower() == 'mkv':
                    output_format = "ts → mkv"
                elif fmt.lower() == 'flv':
                    output_format = "ts → flv"
                else:
                    output_format = "ts → mp4"

            except Exception as e:
                # 如果读取失败，保持默认值，并在日志中记录（如果log方法可用）
                pass
