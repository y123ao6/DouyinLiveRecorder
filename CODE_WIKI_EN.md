...
| h2 | >=4.3.0 | Runtime dependency of httpx `http2=True` (`import h2` inside `Client.__init__`) |
...
- **Module: `DouyinLiveRecorder.egg-info` (build artifact, not committed)** — regenerated via setuptools `egg_info`: `requires.txt` now carries `h2>=4.3.0` / `socksio>=1.0.0`, `SOURCES.txt` no longer lists deleted modules; `PKG-INFO` version stays `4.3.0`.
