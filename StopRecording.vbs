...
Dim objWMIService, colProcesses, objProcess, objShell
Dim intResponse
strComputer = "."
...
If intResponse = vbYes Then
    ...
    If Err.Number <> 0 Then
        MsgBox "无法连接WMI服务，错误代码: " & Err.Number, vbCritical, "错误"
        Err.Clear
        WScript.Quit(1)
    End If
    ...
    Else
        If Not objShell Is Nothing Then
            objShell.Run "taskkill /f /im ffmpeg.exe", 0, True
        End If
    End If
    MsgBox "已成功结束正在录制直播的进程！" & vbCrLf & "关闭此窗口30秒后自动停止录制程序", vbInformation, "提示信息"

    WScript.Sleep 10000
    If Not colProcesses3 Is Nothing Then
        If colProcesses3.Count > 0 Then
            Set colProcesses_ = colProcesses3
        ElseIf Not colProcesses2 Is Nothing And colProcesses2.Count > 0 Then
            Set colProcesses_ = colProcesses2
        Else
            Set colProcesses_ = Nothing
        End If
    Else
        If Not colProcesses2 Is Nothing And colProcesses2.Count > 0 Then
            Set colProcesses_ = colProcesses2
        Else
            Set colProcesses_ = Nothing
        End If
    ...
    If Not colProcesses_ Is Nothing Then
